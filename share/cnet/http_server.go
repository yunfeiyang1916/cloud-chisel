...
func (h *HTTPServer) GoListenAndServer(addr string, handler http.Handler) error {
...
func (h *HTTPServer) GoListenAndServeContext(ctx context.Context, addr string, handler http.Handler) error {
	...
	return h.GoServer(ctx, l, handler)
}

func (h *HTTPServer) GoServer(ctx context.Context, l net.Listener, handler http.Handler) error {
	if ctx == nil {
		return errors.New("ctx must be set")
	}
	h.waiterMux.Lock()
